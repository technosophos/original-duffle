...
//
// Keep in mind that invocation images may do many time-consuming things. This value should
// typically be set high enough to allow multiple VMs to be created.
...
func (d *Kubernetes) Config() map[string]string {
	return map[string]string{
		...
		"SIMULATE":       "If 1, simulate an install, but stop before performing the action",
	}
}
...
// genName generates a Kubernetes-safe name for a secret.
...
// quickHash is NOT cryptographically strong.
//
// It's designed to compute a secret name that is relatively collision resistent inside of the
// particular secret. We optimize for speed.
func quickHash(str string) string {
	return fmt.Sprintf("%x", fnv.New32a().Sum([]byte(str)))
}
...
func (d *Kubernetes) runPodAndWait(name string, op *Operation) error {
	...
	// Wait for the pod to run to completion.
	if err := d.waitForPod(name, op); err != nil {
		return err
	}

	return d.podLog(name, op.Out)
}
	...
	for {
		...
		case e := <-res:
			...
			case "DELETED":
				...
				return fmt.Errorf("pod %s was deleted unexpectedly", name)
			case "ADDED", "MODIFIED":
				pod := e.Object.(*v1.Pod)
				...
				case "Running", "Succeeded":
					// If we are either running or in succeeded, we can bail on this loop and stream the logs.
					req.Stop()
					return nil
				case "Failed":
					req.Stop()
...
func (d *Kubernetes) podLog(name string, w io.Writer) error {
	req := d.Client.CoreV1().Pods(d.Namespace).GetLogs(name, &v1.PodLogOptions{Follow: true})

	readCloser, err := req.Timeout(Timeout).Stream()
	if err != nil {
		return err
	}
